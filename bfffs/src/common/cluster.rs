...
impl<'a> FreeSpaceMap {
    ...
    fn deserialize<V: VdevRaidApi>(vdev: V, buf: DivBuf, zones: ZoneT)
        -> bincode::Result<impl Future<Item=(Self, V), Error=Error>>
    {
    ...
    fn open<V: VdevRaidApi>(vdev: V)
        -> impl Future<Item=(Self, V), Error=Error>
    {
...
impl<'a> Cluster {
    ...
    fn new<V: VdevRaidApi>(args: (FreeSpaceMap, V)) -> Self {
        let (fsm, vdev) = args;
    ...
    pub fn open<V>(vdev_raid: V) -> impl Future<Item=Self, Error=Error>
        where V: VdevRaidApi
    {
...
mod cluster {
    ...
    fn free_and_erase_full_zone() {
        ...
        let cluster = Cluster::new((fsm, vr));
    ...
    fn freespacemap_open() {
        ...
        let (fsm, _mock_vr) = FreeSpaceMap::open(vr).wait().unwrap();
    ...
    fn freespacemap_open_300_zones() {
        ...
        let s = Scenario::new();
        let vr = s.create_mock::<MockVdevRaid>();
        s.expect(vr.zones_call().and_return_clone(300).times(..));
        s.expect(vr.read_spacemap_call(matchers::ANY, 0)
                 .and_call(|mut dbm: DivBufMut, _idx: u32| {
        ...
        s.expect(vr.zone_limits_call(matchers::ANY)
                 .and_call_clone(|zid: ZoneT| {
                     let i = LbaT::from(zid);
                     (100 * i + 4, 100 * i + 96)
                 }).times(..)
        );
    ...
    fn freespacemap_open_ecksum() {
        ...
        let r = FreeSpaceMap::open(vr).wait();
