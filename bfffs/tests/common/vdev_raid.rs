...
test_suite! {
    ...
    const GOLDEN_VDEV_RAID_LABEL: [u8; 168] = [
        ...
        0x68, 0xf6, 0xcf, 0x8f, 0x0e, 0xe8, 0x47, 0x89,
        0xa5, 0x60, 0x17, 0x62, 0x85, 0x2b, 0x64, 0x5d,
        ...
        // UUID is 64-bits long (plus the 8-byte length due to the
        // aforementioned bug in UUID
    ...
    test write_label(raid()) {
        ...
        for path in raid.val.2 {
            ...
            f.seek(SeekFrom::Start(80)).unwrap();   // Skip the VdevLeaf label
            ...
            assert_eq!(&v[24..40], &GOLDEN_VDEV_RAID_LABEL[24..40]);
            // Rest of the buffer should be zero-filled
            assert!(v[168..].iter().all(|&x| x == 0));
        }
    }
}
